...
export abstract class MemoryManagerEmbeddingOps extends MemoryManagerSyncOps {
  ...
  protected async indexFile(
    ...
    // Broadcast memory updates to mesh if it's a memory source and mesh is enabled
    if (options.source === "memory" && entry.path.endsWith(".md")) {
      const { broadcastMeshMemory } = await import("../gateway/mesh-extension.js").catch(
        () => ({ broadcastMeshMemory: null }),
      );
      if (broadcastMeshMemory) {
        void broadcastMeshMemory({
          docId: entry.path,
          content,
        }).catch((err: unknown) => log.error(`Mesh memory broadcast failed: ${String(err)}`));
      }
    }

    // FTS-only mode: skip embedding indexing if no provider
    if (!this.provider) {
